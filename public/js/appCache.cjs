            ...
            switch (_a.label) {
                ...
                case 4:
                    _a
                        ...
                        .push([4, 8,, 9]);
                    return [
                        4, Promise.all(fileList.files.map(fetchAndBust))
                    ];

                case 5:
                    responses = _a.sent();
                    return [
                        4, caches.open(staticCacheName)
                    ];

                case 6:
                    cache_1 = _a.sent();
                    return [
                        4, Promise.all(responses.map(function (response, i) {
                            if (!response.ok) {
                                throw Error("Not ok");
                            }
...
self
    .addEventListener("install", function (event) {
        event.waitUntil(updateCheck().catch(function () {
            return null;
        }));
    });
...
self.addEventListener("fetch", function (event) {
    var responsePromise = caches
        ...
        .then(function (cacheNames) {
            var staticCacheNames = cacheNames.filter(function (n) {
                return n.startsWith("static-separate-list-v");
            });
                ...
                .then(function () {
                    ...
                    return clients
                        ...
                        .then(function (clients) {
                            ...
                            return Promise.all(staticCacheNames.slice(0, -1).map(function (c) {
