...
class Cache extends BaseCommand {
    ...
    static name = 'cache'
    static params = ['cache']
    static usage = ['add <package-spec>', 'clean [<key>]', 'ls [<name>@<version>]', 'verify']static ignoreImplicitWorkspace = true
  ...
  with --force.`)
        ...
        for (const key of args) {
            ...
            await cacache
                .rm
                .content(cachePath, entry.integrity)
        }
    }

    // npm cache add <tarball-url>... npm cache add <pkg> <ver>... npm cache add
    // <tarball>... npm cache add <folder>...
    async add(args) {
        ...
        return Promise.all(args.map(spec => {
            ...
            // we ask pacote for the thing, and then just throw the data away so that it
            // tee-pipes it into the cache like it does for a normal request.
            return pacote
                .tarball
                .stream(spec, stream => {
                    stream.resume()
    ...
    async verify() {
        ...
        if (stats.reclaimedCount) {
            ...
            this
                .npm
                .output(`Content garbage-collected: ${stats.reclaimedCount} (${stats.reclaimedSize} bytes)`)
        }
        if (stats.missingContent) {
            this
                .npm
                .output(`Missing content: ${stats.missingContent}`)
        }
        this
            .npm
